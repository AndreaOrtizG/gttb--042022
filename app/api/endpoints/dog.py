...
from app.schemas.dog_schema import DogBase, DogData, DogInfo, UpdateDog
...
async def update_dogs(
    dog_to_update: UpdateDog,
    name: str,
    current_user: User = Depends(security_login.get_current_active_user),
):
    dog_to_updated = await dog_service.update_dogs(
        update=dog_to_update, name=name, user=current_user
    )
    if dog_to_updated:
...
async def delete_dog(
    name: str, current_user: User = Depends(security_login.get_current_active_user)
):
...
async def add_dog(
    ...
    dog_created = await dog_service.create_dogs(
        dog_created=dog, name=name, user_creator=current_user
    )
