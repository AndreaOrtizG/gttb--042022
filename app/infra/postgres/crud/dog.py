...
class DogCrud(CrudBase[Dog, CreateDog, UpdateDog]):
    async def create_dog(self, *, obj_in: CreateDog) -> Union[dict, None]:
        ...
        if name_existed:
            raise HTTPException(
                status_code=409, detail="Already exists dog with the same name"
            )
        dog_created = await self.model.create(**info_dog)
        return dog_created

    async def update(
        self, *, name: str, dog_updated: UpdateDog, user: User
    ) -> Union[dict, None]:
        dog_info = await self.get_by_field(name=name)
        if dog_info:
            if (
                user.id == dog_info.user_adopter_id
                or user.id == dog_info.user_creator_id
            ):
                dog_to_update = await self.model.filter(name=name).update(
                    **dog_updated.dict()
                )
    ...
    async def delete(self, *, name: str, user: User) -> Union[dict, None]:
        ...
        if dog_to_delete:
            if (
                user.id == dog_to_delete.user_adopter_id
                or user.id == dog_to_delete.user_creator_id
            ):
                dog_to_delete = await self.model.filter(name=name).first().delete()
            else:
                raise HTTPException(status_code=401, detail="User not Authorized")
