...
from app.schemas.general import UpdateSchemaType
from app.services.api_get_random_image import get_dog_picture
from app.infra.postgres.crud.dog import dog_crud, DogCrud
...
class ServiceDog:
    ...
    async def update_dogs(self,*,update: UpdateDog, name:str, user: User):
        ...
        dog_to_update= await self.query.update(name= name, dog_updated=update, user= user)
    ...
    async def create_dogs(self,*, dog_created: DogBase,name: str, user_creator: UserInfo ) -> Union[dict, None]:
        picture_update= get_dog_picture()
        dog_to_create= CreateDog(id=dog_created.id, name=name, picture= picture_update, is_adopted=dog_created.is_adopted)
        if dog_created.user_adopter_email: 
            adopter = await user_service.find_user_email(email=dog_created.user_adopter_email)
